...
class SourceMapGenerator:
    ...
    def add_corridor(self, room1, room2, width=96, height=128, textures=None, add_doors=True):
        ...
        corridor_maxs = [0, 0, 0]
        door_positions = []
        door_angles = (0, 0, 0)  # Default door angles
        ...
        if dx > dy:  # X-axis corridor
            ...
            # Door positions for X-axis corridor (left and right ends)
            door_positions = [
                [corridor_mins[0] + width/4, (corridor_mins[1] + corridor_maxs[1]) / 2, corridor_mins[2] + height/2],  # Left door
                [corridor_maxs[0] - width/4, (corridor_mins[1] + corridor_maxs[1]) / 2, corridor_mins[2] + height/2]   # Right door
            ]
            door_angles = (0, 90, 0)  # Rotate for X-axis corridor (facing perpendicular to corridor)
            ...
            # Door positions for Y-axis corridor (top and bottom ends)
            door_positions = [
                [(corridor_mins[0] + corridor_maxs[0]) / 2, corridor_mins[1] + width/4, corridor_mins[2] + height/2],  # Bottom door
                [(corridor_mins[0] + corridor_maxs[0]) / 2, corridor_maxs[1] - width/4, corridor_mins[2] + height/2]   # Top door
            ]
            door_angles = (0, 0, 0)  # Default orientation for Y-axis corridor
        ...
        # Add doors at both ends of the corridor
        if add_doors:
            door_size = (min(width * 0.8, 64), 4, min(height * 0.8, 128))  # Scale doors to fit corridor
            
            # Add both doors
            for door_pos in door_positions:
                self.add_door(door_pos, door_size, door_angles)
