...
class SourceMapGenerator:
    ...
    def create_simple_room_scenario(self, room_count=3, connect_all=True):
        ...
        # Initialize room coordinates to track already placed rooms
        room_positions = []
        
        # Create rooms in a clearer pattern with explicit spacing
        distance_from_center = max(base_room_size[0], base_room_size[1]) * 3  # Multiply by 3 for adequate spacing
        z_offset = 0  # No vertical stacking
        
        # Create rooms in a circular pattern
        for i in range(room_count):
            angle = (2 * math.pi * i) / room_count
            
            # Calculate position for this room with explicit coordinates
            pos_x = int(base_position[0] + distance_from_center * math.cos(angle))
            pos_y = int(base_position[1] + distance_from_center * math.sin(angle))
            pos_z = base_position[2] + z_offset
            
            # Store room position for later reference
            ...
            # Add room using the explicit position from room_positions array
